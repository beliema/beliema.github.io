namespace L08_Canvas_Alley {
    ...
    function handleLoad(_event: Event): void {
        ...
        drawBackground();
        drawSun({ x: 100, y: 75 });  //Koordinaten der Sonne
        drawCloud({ x: 500, y: 125 }, { x: 250, y: 75 }); //Koordinaten der Wolke
    ...
    function drawTrees(
        _nTrees: number,  //Anzahl von Bäumen 
        _posStart: Vector,  //Startsposition hinten
        _posEnd: Vector,  //Endposition vorne
        _minScale: number,  // 
        _stepPos: number,   // jetzige Position des Baumes
        _stepScale: number): // Abstand zwischen Bäumen 
        void {
    ...
    function drawBackground(): void {
        ...
        let gradient: CanvasGradient = crc2.createLinearGradient(0, 0, 0, crc2.canvas.height); //Farbverlauf im Hintergrund
    ...
    function drawSun(_position: Vector): void {
        ...
        let r1: number = 30; //innerer Kreis
        let r2: number = 150; //äußerer Kreis
        let gradient: CanvasGradient = crc2.createRadialGradient(0, 0, r1, 0, 0, r2);

        gradient.addColorStop(0, "HSLA(60, 100%, 90%, 1)");
        gradient.addColorStop(1, "HSLA(60, 100%, 50%, 0)");

        crc2.save();  //Infos werden gespeichert
        crc2.translate(_position.x, _position.y);
        crc2.fillStyle = gradient;
        crc2.arc(0, 0, r2, 0, 2 * Math.PI);
        crc2.fill();
        crc2.restore(); //nimmt letzten gespeicherten Wert
    }
    ...
    function drawCloud(_position: Vector, _size: Vector): void {
        ...
        gradient.addColorStop(0, "HSLA(0, 100%, 100%, 0.5)"); // 0.5 -> transparenter, 0 -> solid
